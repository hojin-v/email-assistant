...
이 문서는 EmailAssist의 서비스 구조, 화면 흐름, 데이터, API, AI, 인프라를 하나의 설계서로 정리한 문서다. `docs/목차.md`의 장/절 체계를 그대로 따르며, 현재 코드와 문서에서 확인 가능한 사실을 최우선으로 사용한다.

문서 내 표기 규칙은 다음과 같다.

- `확정 정보`: 현재 코드나 문서에서 검증 가능한 사실
- `임시 작성`: 목차를 채우기 위해 합리적으로 보강한 내용. 이후 수정 가능
- `추가 정보 필요`: 실제 구현 전에 사용자 또는 팀 의사결정이 더 필요한 항목
- 문서 간 충돌이나 추가 확인이 필요한 내용은 `concept-conflicts.md`와 이 문서의 `추가 정보 필요` 항목을 함께 본다.
...
| 영역                 | 상태   | 비고                        |
| -------------------- | ------ | --------------------------- |
| `frontend/App`       | 구현됨 | 사용자용 웹 앱 프로토타입   |
| `frontend/Admin-Web` | 구현됨 | 관리자 콘솔 임시 프로토타입 |
| `backend`            | 미구현 | 문서 설계 단계              |
| `ai`                 | 미구현 | 문서 설계 단계              |
| `docs`               | 구현됨 | 설계와 컨텍스트 문서        |

프론트엔드 개발 범위는 현재 `frontend/` 하위 디렉터리로 한정한다.
...
현재 저장소는 프론트 프로토타입과 설계 문서가 먼저 존재하고, 백엔드와 AI는 미구현 상태다. 따라서 일정은 코드 완료일보다 산출물 단계로 이해하는 것이 맞다.

현재 주요 산출물은 다음과 같다.

- `frontend/App`: 사용자 앱 프로토타입
- `frontend/Admin-Web`: 관리자 콘솔 임시 프로토타입
- `docs/architect.md`: 통합 설계 문서
- `docs/agents.md`: 구현 핸드오프 문서
- `docs/concept-conflicts.md`: 충돌 및 후속 정보 요청 문서
...
| 단계  | 목적                          | 산출물                                  |
| ----- | ----------------------------- | --------------------------------------- |
| 1단계 | 서비스/화면/문서 기준 확정    | 본 문서, `agents.md`, 프론트 프로토타입 |
| 2단계 | 백엔드/AI 인터페이스 확정     | REST API, 내부 AI API, DB 설계          |
| 3단계 | 사용자 앱 API 연동            | App 실제 데이터 연동                    |
| 4단계 | 관리자 콘솔 방향 확정 및 연동 | Admin-Web 요구사항 확정, API 연동       |
...
문서와 저장소 기준 역할은 다음과 같이 분리된다.

| 역할       | 책임                                                            |
| ---------- | --------------------------------------------------------------- |
| 프론트엔드 | `frontend/` 하위 UI, 상태 관리, 화면 흐름, API 연동             |
| 백엔드     | 인증, Gmail/Calendar 연동, 저장, 관리자 API, 메시지 브로커 조율 |
| AI         | 데이터셋 구축, 분류기, 요약/엔터티 추출, RAG, 초안 생성         |
| 인프라     | 네트워크, 스토리지, 브로커, 배포, 보안 정책                     |

`frontend/App`은 현재 구현 사실이 존재하므로 설계보다 코드가 우선이다.

`frontend/Admin-Web`은 현재 라우트와 화면 구조는 사실로 볼 수 있지만, 기능 범위와 상세 동작은 임시안이다.

**임시 작성**

협업 방식은 아래처럼 가정한다.

- 프론트엔드는 사용자 앱을 먼저 실제 서비스에 연결한다.
- Admin-Web은 요구사항이 정리되기 전까지 정보구조 참고용으로 유지한다.
- 백엔드는 프론트가 기대하는 공개 API와 AI 내부 API의 중간 조율 계층을 담당한다.
- AI는 독립 서비스 또는 내부 워커로 구현될 수 있다.

**추가 정보 필요**

- 관리자 콘솔 소유 팀과 승인 프로세스
- 백엔드와 AI를 한 팀이 구현하는지, 분리된 팀이 구현하는지
...
현재 사용자 앱은 웹 기준 구현이며, 반응형 레이아웃을 포함한다.

#### 현재 라우트와 구현 상태

| 경로              | 현재 구현         | 현재 데이터 소스                          | 목표 상태                            |
| ----------------- | ----------------- | ----------------------------------------- | ------------------------------------ |
| `/`               | `FirstTimeSetup`  | 로컬 상태                                 | 초기 설정/로그인 전 흐름 정리 필요   |
| `/app`            | 대시보드          | 더미 통계, `emailItems`, `calendarEvents` | 실제 API 연동                        |
| `/app/inbox`      | 수신함            | `emailItems`                              | 실제 메일/분석/초안 API              |
| `/app/calendar`   | 캘린더            | 로컬 이벤트 상태                          | 실제 일정 API + Calendar 연동        |
| `/app/templates`  | 템플릿 라이브러리 | 로컬 템플릿 상태                          | 실제 템플릿 API                      |
| `/app/automation` | 자동화 설정       | 로컬 규칙 상태                            | 실제 자동화 규칙/연동 상태 API       |
| `/app/profile`    | 비즈니스 프로필   | 로컬 상태                                 | 실제 비즈니스 프로필/자료/재생성 API |
| `/app/settings`   | 설정              | 로컬 기본 설정                            | 실제 계정/알림/연동/문의 API         |
| `/app/onboarding` | 온보딩 위저드     | 로컬 상태                                 | 실제 저장 + 재생성 요청              |

#### 사용자 앱 주요 화면별 현재 사실

1. **초기 설정 `/`**
   - 이메일 연결을 버튼 클릭으로 시뮬레이션한다.
   - UI의 `업종 / 비즈니스 유형` 선택지는 내부 `Domain` 목록에 대응하며, 화면에는 `한글 (영문)` 형식으로 표시한다.
   - 선택한 업종 / 비즈니스 유형에 해당하는 카테고리는 처음부터 전부 로컬 상태에 등록된다.
   - `새 카테고리 추가` 입력창을 누르면 다른 업종의 추천 카테고리 목록이 드롭다운으로 열리고, 직접 임의 카테고리 입력도 가능하다.
   - 설명, 어조, 업로드 파일, FAQ를 로컬 상태로 관리한다.
   - 템플릿 생성은 타임아웃 기반 진행 UI만 제공한다.
   - 실제 OAuth, 파일 업로드, 저장 API는 없다.

2. **대시보드 `/app`**
   - 상단 카드: 처리 이메일 수, 검토 대기 초안, 템플릿 매칭률, 계정 상태
   - 다가오는 일정: `calendarEvents`
   - 최근 수신 이메일: `emailItems`
   - 이번 주 요약: 고정 차트 데이터
...
3. **수신함 `/app/inbox`**
   ...
   - 3패널 구조: 메일 목록, 본문, 초안
   - 액션: 발송, 편집 후 발송, 건너뛰기
   - 액션 결과는 로컬 상태 업데이트만 수행한다.
   - 일정 감지 카드에서 캘린더로 `prefillEvent`를 전달한다.

4. **캘린더 `/app/calendar`**
   - 로컬 `CalendarEvent` 상태를 관리한다.
...
6. **자동화 설정 `/app/automation`**
   ...
   - 캘린더 연결 상태와 자동 등록 카테고리를 로컬 상태로 관리한다.
   - 현재 구현은 AI 분류 정책이 아니라 키워드 규칙 중심이다.

7. **비즈니스 프로필 `/app/profile`**
   - 업종, 설명, 어조, 업로드 파일, FAQ를 로컬 상태로 관리한다.
   - 템플릿 재생성은 시뮬레이션 토스트로 표현된다.

8. **설정 `/app/settings`**
   - 탭 구조: `account`, `notifications`, `display`, `email`, `support`
   - 연결된 이메일 계정도 로컬 배열 상태다.
   - 관리자 문의는 `SupportTickets` 구조(`title`, `content`, `status`, `admin_reply`)를 따르는 로컬 목록/상세 UI로 표현된다.
   - 실제 연동 해제나 OAuth 연결은 없다.
...
완성형 사용자 앱은 아래 기능을 추가로 가져야 한다.

- 실제 인증/로그인
- 서버 기반 메일 목록/상세/초안 조회
- 실제 Gmail 발송
...
현재 `frontend/Admin-Web`은 아래 라우트를 가진 임시 구현이다.

| 경로         | 현재 사실               |
| ------------ | ----------------------- |
| `/`          | 운영 대시보드 임시 화면 |
| `/members`   | 회원 관리 임시 화면     |
| `/templates` | 템플릿 관리 임시 화면   |
| `/inquiries` | 문의 대응 임시 화면     |

현재 구현은 모두 더미 데이터 기반이며, 실제 API 연동은 없다.

**임시 작성**

Admin-Web의 임시 목표안은 아래 범위를 포함한다.
...
이 내용은 제품 요구사항이 확정된 것이 아니라, 현재 Notion과 프로토타입을 바탕으로 임시 정리한 것이다.
...
첨부파일 처리 흐름은 아래처럼 구현하는 것을 기준으로 둔다.
...
현재 `/`는 로그인 화면이 아니라 `FirstTimeSetup` 화면이다.
...
- 경로: `/app`
- 데이터: 더미 통계, `emailItems`, `calendarEvents`
- 구현: 카드, 일정 리스트, 최근 메일 리스트, 주간 분포 차트
...
- 데스크톱: 3컬럼 레이아웃
- 모바일: 목록/상세 전환 구조
- 현재 메일 상태값: `pending`, `completed`, `auto-sent`
...
현재 화면은 모두 더미 데이터 기반이고, 관리 액션은 대부분 비활성 또는 표시용이다.
...
이 내용은 현재 정보구조 참고용이며 최종 요구사항은 아니다.
...
Admin Frontend (frontend/Admin-Web)
        |
        v
   Backend API
        |
...
현재 관리자 기능은 임시 구현이므로 아래 시퀀스도 임시 목표안이다.
...
REST API 기준은 `docs/notion_back1.md`, `docs/notion_back2.md`의 구체 정의를 반영한다. 사용자 앱과 관리자 앱 공개 API는 `/api`, AI 내부 호출은 `/internal/ai` 기준으로 분리한다.
...
서비스 정규 저장소는 MariaDB를 기준으로 한다. 아래 명세는 `docs/notion_db.md`의 테이블 정의를 설계서 수준으로 정리한 것이다.
...
- 관계: `Users.user_id` FK
- 구현 메모: 실제 파일 바이너리는 외부 저장소 또는 파일시스템에 두고, DB에는 메타데이터와 경로만 저장한다.
...
- 구현 메모: `body_template`의 `{{변수명}}` 패턴은 변수 추출과 UI 강조에 사용한다.
...
- 구현 메모: `variables_info`는 MariaDB JSON 호환 컬럼을 사용한다.
...
- 구현 메모: AI 입력의 기준 문자열은 `email_text = subject + body_clean`이다.
...
- 구현 메모: `domain`, `intent`는 Notion AI taxonomy를 기준으로 저장하고, `category_id`는 사용자 운영 카테고리 매핑에 사용한다.
...
- 구현 메모: `EDIT_SEND` 액션 시 본문 수정본을 저장하고 상태를 `EDITED` 또는 발송 후 `SENT`로 전환한다.
...
- 구현 메모: Google Calendar 연동 성공 여부는 `is_calendar_added`로 관리한다.
...
- 구현 메모: `keywords`는 MariaDB JSON 호환 컬럼 사용, 규칙 생성 시 `Categories`와 트랜잭션 처리 필요
...
- `frontend/App`: TypeScript-first
- `frontend/Admin-Web`: JSX 유지

**임시 작성**

문서와 코드의 상태 표시는 아래 표현을 유지한다.
