...
- 수신함은 `draft_status`를 목록 단계에서 이미 연결했다. 남은 주요 이슈는 `sender_email`, 고정 `schedule` 구조를 UI가 직접 소비하도록 정리하는 것이다. 상세 내용은 [inbox-backend-gap.md](./inbox-backend-gap.md)에 정리했다.
...
2. 수신함이 `sender_email`과 고정 `schedule` 구조를 직접 소비하도록 수정
3. 관리자 나머지 화면 연결
